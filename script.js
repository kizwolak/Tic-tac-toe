...
const gameplay = (() => {
  ...
  const randomTurn = (p1, p2) => {
    if (p1.turn > p2.turn) {
      return (p1.marker = "O"), (p2.marker = "X"), (currentPlayer = p1);
    } else if (p1.turn < p2.turn) {
      return (p1.marker = "X"), (p2.marker = "O"), (currentPlayer = p2);
    }
  };
  ...
  const victory1 = (p1) => {
    containerClear(container);
    const victory1p = document.createElement("div");
  ...
  const victory2 = (p2) => {
    containerClear(container);
    const victory2p = document.createElement("div");
  ...
  const winCheck = (p1, p2) => {
    ...
    if (
      ((p1Cells.length === 5 && p2Cells.length === 4) ||
        (p1Cells.length === 4 && p2Cells.length === 5)) &&
      victoryExists === false
    ) {
      draw();
    }
